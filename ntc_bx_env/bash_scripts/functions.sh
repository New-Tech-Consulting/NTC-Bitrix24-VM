...
main_menu(){
    ...
    if [ -f "/tmp/new_version_menu.tmp" ]; then
      ...
      msg_new_version_menu="\e[33m          New version of NTC Bitrix-ENV for DEBIAN12
          (your version ${BS_VERSION_MENU} -> new version ${nv}) please enter
          \"update_ntc_env\" to update your environment\n\e[0m"
      update_menu_action="Enter \"update_ntc_env\" to update your menu";
    fi

    echo -e "           New Tech Consulting \"Bitrix-ENV for DEBIAN12\" version ${BS_VERSION_MENU}         \n\n";
    if [ $BS_SHOW_IP_CURRENT_SERVER_IN_MENU = true ]; then
